...
Maven build (`pom.xml`), compiled with **`javac`** via `maven-compiler-plugin` targeting **Java 11** (`<maven.compiler.release>11</maven.compiler.release>`). Minimum consumer runtime: **JDK 11**. Build JDK: 17+ (JDK 25 in use). Version is `2.1.1`.
